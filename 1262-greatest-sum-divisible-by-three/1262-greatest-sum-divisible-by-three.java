class Solution {
    public int maxSumDivThree(int[] nums) {
        ...
        if (sum % 3 == 1) {
            ...
            if (!rem1.isEmpty()) op1 = sum - rem1.get(0);
            if (rem2.size() >= 2) op2 = sum - rem2.get(0) - rem2.get(1);
            ans = Math.max(op1, op2);
        }
        else {
            int op1 = Integer.MIN_VALUE;
            int op2 = Integer.MIN_VALUE;
            if (!rem2.isEmpty()) op1 = sum - rem2.get(0);
            if (rem1.size() >= 2) op2 = sum - rem1.get(0) - rem1.get(1);
            ans = Math.max(op1, op2);
        }
        return ans < 0 ? 0 : ans;
    }
